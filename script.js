...
(async function () {
  ...
  function renderProducts(filter = 'todos') {
    ...
    if (visible.length === 0) {
      grid.innerHTML = `<p style="text-align:center;color:var(--text-muted);padding:40px;grid-column:1/-1">No hay productos en esta categoría por ahora.</p>`;
      return;
    }
    ...
    grid.querySelectorAll('.btn-ver-detalles').forEach(btn => {
      btn.addEventListener('click', () => {
